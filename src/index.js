 ...
 */
function categorizeTechStack(detectedTechnologies = []) {
  const categories = {
    frontend: [],
  ...
  detectedTechnologies.forEach((tech) => {
    const name = tech.technology || tech.name || tech;
    const cat = (tech.category || '').toLowerCase();

    if (/frontend|framework|ui|css|react|next|vue/i.test(cat) || /next\.js|react|vue|tailwind/i.test(name)) {
      categories.frontend.push(name);
    } else if (/hosting|cdn|server|cloud|dns/i.test(cat) || /vercel|cloudflare|aws|railway|netlify/i.test(name)) {
      categories.hostingAndInfrastructure.push(name);
    } else if (/analytics|marketing|tracker|pixel/i.test(cat) || /google analytics|meta pixel|posthog|segment|linkedin/i.test(name)) {
      categories.analyticsAndMarketing.push(name);
    } else if (/payment|billing|finance/i.test(cat) || /stripe|paystack|chargebee|lemon squeezy/i.test(name)) {
      categories.paymentsAndBilling.push(name);
    } else {
      categories.utilitiesAndLibraries.push(name);
    }
  });
...
function formatReportPayload(site, rawOutput, reportData, seoData = {}) {
  ...
  return {
    ...
    businessSummary:
      ...
      reportData?.summary ||
      `${site.name} platform competitive analysis.`,
    valuePropositions: reportData?.valuePropositions || [],
    targetPersona: reportData?.targetPersona || {
      primaryAudience: 'N/A',
      idealCustomerProfile: 'N/A',
      keyPainPoints: [],
    },
    revenueModel: reportData?.revenueModel || {
    ...
    techStack: reportData?.techStack || detectedTech.map((t) => t.technology),
    categorizedTechStack: categorizedTech,
    technicalArchitecture: reportData?.technicalArchitecture || {
      frontend: categorizedTech.frontend.join(', ') || 'N/A',
      backendAndAnalytics: [
        ...categorizedTech.analyticsAndMarketing,
        ...categorizedTech.paymentsAndBilling,
      ],
      infrastructureInsights:
        categorizedTech.hostingAndInfrastructure.join(', ') || 'N/A',
    },
...
export async function teardownSite(site, options = {}) {
  ...
  if (format === 'json') {
    ...
    try {
      const exportedPath = await exportReport(finalReport, {
        format,
        outdir: outputDir,
        outputDir,
      });
  ...
  .action(async (options) => {
    try {
      const jsonContent = await fs.readFile(
        path.join(projectRoot, 'test-sites.json'),
        'utf-8'
      );
